...
SELECT stockCode, quantity, unitPrice, ROUND((quantity * unitPrice), 2) AS totalAmount, countryName
FROM outdoorProducts
where quantity>0;
...
FROM salesQuants 
JOIN countryCodes ON countryName = EnglishShortName
...
SELECT totalQuantity, alpha3Code
FROM salesQuants 
JOIN modCountryCodes ON countryName = EnglishShortName
