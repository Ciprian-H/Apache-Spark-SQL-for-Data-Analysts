...
create or replace temporary view q1results as
  select TrueFalse, count(*) as count from revenue1
  group by TrueFalse
...
create or replace temporary view q2results as
  select TrueFalse, max(Amount) as MaxAmount from revenue2
  group by TrueFalse
...
select * from revenue3

-- COMMAND ----------

-- TODO  Answer 3
create or replace temporary view q3results as
  select TrueFalse, avg(Amount) as avgAmount from revenue3
  group by TrueFalse
...
select * from revenue4 tablesample (6 rows)
...
create or replace temporary view q4results as
  ...
  from (select Year, YesNo, Amount
        from (select year(cast(UTCTime as timestamp)) as Year, YesNo, Amount from revenue4)
...
create or replace temporary view q5results as
  select aisle, sum(amount) as amount from products
...
create or replace temporary view q6Results as
  select coalesce(itemName, "All iteams") as itemName, coalesce(month(date), "All months") as month, avg(revenue) as avgRevenue
  from sales
