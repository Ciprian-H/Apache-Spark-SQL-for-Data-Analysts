...
with ExploadeSource
as (
  select explode(authors) as authors, categories from databricksBlog
  )
select distinct authors, categories from ExploadeSource
where array_contains(categories, "Company Blog")

-- COMMAND ----------

create or replace temporary view results as
select distinct (explode(authors)) as author, categories
from databricksBlog
where array_contains(categories, "Company Blog")
