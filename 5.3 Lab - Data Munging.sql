...
--TODO
select * from purchaseEvents sort by purchases desc
...
select round(avg(purchases),1) AvgPurchases, eventDate DayOfPurchase from purchaseEvents group by DayOfPurchase

-- COMMAND ----------

--TODO
--total purchases by state
SELECT round(sum(purchases), 2) totalPurchases, state day FROM purchaseEvents GROUP BY state

-- COMMAND ----------

--TODO
drop table if exists usersRaw;
create table usersRaw using parquet options(path "/mnt/training/ecommerce/users/users.parquet", header "true");

-- COMMAND ----------

select * from usersRaw

-- COMMAND ----------

--TODO
select purchaseEvents.userId, usersRaw.email from purchaseEvents join usersRaw on purchaseEvents.userId=usersRaw.user_id where email is not null
--select purchases, email from purchaseEvents join usersRaw on purchaseEvents.userId=usersRaw.userId

-- COMMAND ----------

SELECT  purchases, eventDate, city, state, email
FROM purchaseEvents
JOIN usersRaw
ON purchaseEvents.userId=usersRaw.user_id
